...
/obj/structure/toilet/attack_hand(mob/living/user)
	...
	if(lid_open && user.loc == loc)
		...
		if(do_after(user, rand(5, 20) SECONDS, needhand = FALSE, target = src))
			...
			SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "wc_used", /datum/karmatic_factor/wc_used)
			...
			if(prob(problem_chance))
				...
				if(HAS_TRAIT(user, TRAIT_CLUMSY))
					SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "clown_evil", /datum/karmatic_factor/clown_evil)
...
/obj/structure/urinal/attack_hand(mob/living/user)
	if(user.loc == loc) // no gender discrimination here!
		...
		if(do_after(user, rand(5, 10) SECONDS, needhand = TRUE, target = src))
			...
			user.add_karmatic_factor("wc_used", /datum/karmatic_factor/wc_used)
	...
	if(iscarbon(M))
		...
		switch(watertemp)
			if("normal")
				...
				C.AdjustStunned(-1)
				C.AdjustWeakened(-1)
				C.add_karmatic_factor("shower-time", /datum/karmatic_factor/shower)
				return
			if("freezing")
