...
/obj/structure/stacklifter/proc/get_pumped(mob/living/carbon/human/user)
	...
	if(C)
		...
		SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "swole", /datum/karmatic_factor/swole, pain_amount)
	if(G)
...
/obj/item/weapon/dumbbell/attack_self(mob/living/user)
	...
	SEND_SIGNAL(H, COMSIG_ADD_MOOD_EVENT, "swole", /datum/karmatic_factor/swole, pain_amount)
	H.update_body()
