...
/obj/machinery/disposal/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	. = ..()
	if(action == "eject")
		if(HAS_TRAIT(usr, TRAIT_HIDDEN_STASH))
			to_chat(usr, "<span class='notice'>Оп-па! Иди к [usr.gender == FEMALE ? "мамочке" : "папочке"]!</span>")
			usr.put_in_active_hand(new /obj/item/weapon/storage/backpack/satchel/flat/orbital(loc))
			REMOVE_TRAIT(usr, TRAIT_HIDDEN_STASH, ROLE_TRAIT)

/obj/item/weapon/storage/backpack/satchel/flat/orbital/atom_init()
	. = ..()
